        ...
        <ul className="space-y-2">
          {data?.institutions?.length
            ? data.institutions.map((item: any) => (
                <li key={item.holder} className="flex justify-between text-sm">
                  <span>{item.holder}</span>
                  <span className="font-mono">{item.pct.toFixed(2)}%</span>
                </li>
              ))
            : <li className="text-neutral-400">No data available.</li>
          }
        </ul>
      </div>
      <div>
        ...
        <ul className="space-y-2">
          {data?.insiders?.length
            ? data.insiders.map((item: any) => (
                <li key={item.name} className="flex justify-between text-sm">
                  <span className={item.type === 'Sell' ? 'text-red-400' : 'text-green-400'}>{item.type}</span>
                  <span>{item.name}</span>
                  <span className="font-mono">{item.shares.toLocaleString()} Shares</span>
                </li>
              ))
            : <li className="text-neutral-400">No data available.</li>
          }
